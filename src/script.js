...
function generate_question(){
    ...
    const random_operation = operations[Math.floor(Math.random() * operations.length)];
...
function check_user_input(){
    ...
    const user_answer = parseInt(document.getElementById('user-answer').value, 10);
    const feedback_element = document.getElementById('feedback');
    const previous_correct_answer = feedback_element.getAttribute('data-correct-answer');
    let correct_answer;
